...
from sahayak_agent.agent import root_agent
...
def create() -> None:
    ...
    remote_app = agent_engines.create(
        ...
        extra_packages=["./sahayak_agent"],
        display_name="sahayak-agent-engine-v1",
        description="Educational content creation agent for NCERT curriculum",
    )
...
def create_session(resource_id: str) -> None:
    """Creates a new session for the specified user."""
    remote_app = agent_engines.get(resource_id)
    remote_session = remote_app.create_session(user_id="user_123")
...
def main(argv=None):
    ...
    bucket = FLAGS.bucket if FLAGS.bucket else 'gs://sahayak-ai-bot'
    ...
    if FLAGS.create:
        ...
        if not FLAGS.resource_id:
            ...
            return
        create_session(FLAGS.resource_id)
